...
local function getusername(parameters)

	local usercode = tonumber(parameters:sub(2,5))
	log.debug ('getusername: usercode=', usercode)
	if not usercode then
		return parameters:sub(2,5)
		
	else
		return conf.usernames[usercode]
	end

end


local function build_event_table(code, parms, event, message)

	local update = {}
	local parameters = parms.modparms
	...
	if evl.ResponseTypes[code] then
		...
		elseif (code == 502) then
			if parameters == '024' then
				update =	{
								['type'] = 'partition',
								['value'] = '1',
								['status'] = 'cantarm'
							}
			elseif parameters == '023' then
				update =	{
								['type'] = 'partition',
								['value'] = '1',
								['status'] = 'notarmed'
		...
		elseif code == 849 then
			
			update = 	{
									['type'] = 'partition',
									['value'] = '1',
									['status'] = 'verbosetrouble',
									['parameters'] = {}
								}
			...
			for i = 1, 8 do
				if binary:sub(i,i) == '1' then
					update['parameters'][verboseTroubleMap[i]] = 'on'
				else
					update['parameters'][verboseTroubleMap[i]] = 'off'
				end
			end
		...
		elseif event.type == 'partition' then
			...
			if conf.partitions[partnum] then
				if code == 655 then
					send_command('071', '1*1#')						-- refresh zone bypass status
				end
				...
				if (code == 700) or (code == 750) then
					update['username'] = getusername(parms.allparms)
					log.debug('user open/close; username=', update.username)
				end
		...
		if event.type == 'partition' then
			...
			if  partition_name then
				...
				if paramlen == 5 then
					...
					return string.format(event.name, partition_name, getusername(parameters))
...
local function handle_line(input)
    ...
    if code == 500 then
      if timers.pollreset then
        evlDriver:cancel_timer(timers.pollreset)
        timers.pollreset = nil
      end
    end
		...
		local eventtable = build_event_table(code, {modparms=modparms, allparms=parameters}, event, message)
...
local function periodic_check()
  ...
  if connected then       -- if we THINK we're connected, try pinging the Envisalink...
    send_command('000', '')
    -- This timer will be canceled when poll command acknowledgement is received
    timers.pollreset = evlDriver:call_with_delay(5, resetconnection)
  end
